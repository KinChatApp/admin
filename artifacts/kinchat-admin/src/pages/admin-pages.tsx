...
import { Activity, ArrowUpRight, BarChart3, Blocks, CheckCircle2, Clock3, FileWarning, GitBranch, LockKeyhole, MessageSquare, MoreHorizontal, Radio, RefreshCw, Server, ShieldAlert, Users, UserPlus, Video } from 'lucide-react';
import { Link } from 'wouter';
import { TinyLabel } from '@/components/admin-shell';
...
const pageDetails: Record<string, { eyebrow: string; title: string; description: string; icon: typeof Users }> = {
  users: { eyebrow: 'Workspace', title: 'Identity & Users', description: 'Manage the people and identity signals that make KinChat a trusted place to connect.', icon: Users },
  communication: { eyebrow: 'Workspace', title: 'Communication', description: 'Visibility into conversations, delivery, and the health of KinChat messaging.', icon: MessageSquare },
  calls: { eyebrow: 'Workspace', title: 'Calls', description: 'Monitor voice and video reliability across the KinChat network.', icon: Video },
  content: { eyebrow: 'Workspace', title: 'Content & Media', description: 'A future home for media flows, storage, and content lifecycle operations.', icon: FileWarning },
  safety: { eyebrow: 'Governance', title: 'Trust & Safety', description: 'Keep the community safe with clear review queues and responsible enforcement tools.', icon: ShieldAlert },
  engagement: { eyebrow: 'Governance', title: 'Engagement', description: 'Understand the rhythms that keep teams connected and conversations moving.', icon: Activity },
  analytics: { eyebrow: 'Intelligence', title: 'Analytics', description: 'A focused workspace for product signals, trends, and operational reporting.', icon: BarChart3 },
  platform: { eyebrow: 'Infrastructure', title: 'Platform', description: 'Service-level visibility across the systems that power KinChat.', icon: Blocks },
  operations: { eyebrow: 'Infrastructure', title: 'Operations', description: 'Coordinate operational work with a clear, accountable view of the platform.', icon: Server },
  security: { eyebrow: 'Infrastructure', title: 'Security', description: 'Protect the people and systems behind every KinChat conversation.', icon: LockKeyhole },
  developer: { eyebrow: 'Infrastructure', title: 'Developer', description: 'Tools and diagnostics for the teams extending KinChat.', icon: GitBranch },
};

export function ComingSoon({ page }: { page: string }) {
  const detail = pageDetails[page] ?? pageDetails.users;
  const Icon = detail.icon;
  return <div className="panel-grid min-h-full"><div className="mx-auto flex min-h-[calc(100dvh-72px)] max-w-[1100px] items-center justify-center px-5 py-12 sm:px-8"><div className="w-full max-w-xl animate-rise-in text-center"><div className="mx-auto grid h-14 w-14 place-items-center rounded-lg border border-[#1E3A5F] bg-[#0D1B2A] text-[#60A5FA]"><Icon size={24} strokeWidth={1.6} /></div><TinyLabel>{detail.eyebrow} · module</TinyLabel><h1 className="mt-4 text-3xl font-semibold tracking-[-0.04em] text-[#F8FAFC] sm:text-4xl">{detail.title}</h1><p className="mx-auto mt-4 max-w-md text-sm leading-6 text-[#94A3B8]">{detail.description}</p><div className="mx-auto mt-8 flex max-w-sm items-center gap-3 rounded-lg border border-[#1E3A5F] bg-[#0D1B2A] px-4 py-3 text-left"><Clock3 size={16} className="shrink-0 text-[#64748B]" /><div><p className="text-xs font-medium text-[#F8FAFC]">This module is in preparation</p><p className="mt-1 text-[11px] leading-5 text-[#64748B]">The foundation is ready. Data views and actions will arrive here in a future release.</p></div></div><Link href="/admin" className="mt-8 inline-flex items-center gap-2 text-xs font-medium text-[#60A5FA] hover:text-[#93C5FD]" data-testid={`link-return-overview-${page}`}>Return to overview<ArrowUpRight size={14} /></Link></div></div></div>;
}